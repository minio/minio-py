            ...
            if isinstance(ordered_query[component_key], list):
                for value in ordered_query[component_key]:
                    query_components.append(
                        component_key + "=" + queryencode(value)
                    )
            else:
                query_components.append(
                    component_key
...
def is_valid_bucket_notification_config(notifications):
    ...
    VALID_SERVICE_CONFIG_KEYS = set(["Id", "Arn", "Events", "Filter",])
    ...
    for key, value in notifications.items():
        ...
        if key not in VALID_NOTIFICATION_KEYS:
            raise InvalidArgumentError(
                (
                    "{} is an invalid key " "for notifications configuration"
                ).format(key)
            )
        ...
        for service_config in value:
            ...
            if "Filter" in service_config:
                ...
                ).format(service_config["Filter"])
                try:
                    filter_rules = (
                        service_config.get("Filter", {})
                        .get("Key", {})
                        .get("FilterRules", [])
                    )
...
def optimal_part_info(length, part_size):
    ...
    if length > MAX_MULTIPART_OBJECT_SIZE:
        raise InvalidArgumentError(
            "Input content size is bigger " " than allowed maximum of 5TiB."
        )
    ...
    if part_size != MIN_PART_SIZE:
        ...
        part_size_float = math.ceil(part_size_float / part_size) * part_size
    # Total parts count.
...
def amzprefix_user_metadata(metadata):
    ...
    for k, v in metadata.items():
        ...
        if (
            is_amz_header(k)
            or is_supported_header(k)
            or is_storageclass_header(k)
        ):
            m[k] = v
        else:
            m["X-Amz-Meta-" + k] = v
...
def is_amz_header(key):
    ...
    return (
        ...
        or key == "x-amz-acl"
        or key.startswith("x-amz-server-side-encryption")
    )
