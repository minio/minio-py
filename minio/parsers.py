            ...
            try:
                ...
                raise InvalidXMLError(
                    (
                        ...
                        "Message: {}"
                    ).format(self.root_name, name, error)
                )
        else:
...
def parse_list_objects(data, bucket_name):
    ...
    objects, object_dirs = _parse_objects_from_xml_elts(
        bucket_name, root.findall("Contents"), root.findall("CommonPrefixes")
    )
...
def parse_list_objects_v2(data, bucket_name):
    ...
    continuation_token = root.get_child_text(
        "NextContinuationToken", strict=False
    )
    objects, object_dirs = _parse_objects_from_xml_elts(
        bucket_name, root.findall("Contents"), root.findall("CommonPrefixes")
    )
...
def parse_get_bucket_notification(data):
    ...
    notifications = _parse_add_notifying_service_config(
        root, {}, "TopicConfigurations", "TopicConfiguration"
    )
    notifications = _parse_add_notifying_service_config(
        root, notifications, "QueueConfigurations", "QueueConfiguration"
    )
    notifications = _parse_add_notifying_service_config(
        root,
        notifications,
