...
class MinioError(Exception):
    ...
    def __str__(self):
        return "{name}: message: {message}".format(
            name=self.__class__.__name__, message=self.message
        )
...
class MultiDeleteError(object):
    ...
    def __str__(self):
        string_format = (
            "<MultiDeleteError: object_name: {} error_code: {}"
            " error_message: {}>"
        )
        return string_format.format(
            self.object_name, self.error_code, self.error_message
        )
...
class ResponseError(MinioError):
    ...
    def __init__(self, response, method, bucket_name=None, object_name=None):
    ...
    def __str__(self):
        return (
            "ResponseError: code: {0}, message: {1},"
            " bucket_name: {2}, object_name: {3}, request_id: {4},"
            " host_id: {5}, region: {6}".format(
                self.code,
                self.message,
                self.bucket_name,
                self.object_name,
...
class AccountProblem(KnownResponseError):
    message = (
        "There is a problem with your account that prevents the "
...
class AmbiguousGrantByEmailAddress(KnownResponseError):
    message = (
        "The email address you provided is associated with "
...
class CrossLocationLoggingProhibited(KnownResponseError):
    message = (
        "Cross-location logging not allowed. Buckets in one "
...
class EntityTooSmall(KnownResponseError):
    message = (
        "Your proposed upload is smaller than the minimum a"
        "llowed object size."
    )
...
class IllegalVersioningConfigurationException(KnownResponseError):
    message = (
        "Indicates that the versioning configuration specified "
...
class InvalidEncryptionAlgorithmError(KnownResponseError):
    message = (
        "The encryption request you specified is not valid. "
...
class InvalidPartOrder(KnownResponseError):
    message = (
        "The list of parts was not in ascending order.Parts list "
...
class InvalidPolicyDocument(KnownResponseError):
    message = (
        "The content of the form does not meet the conditions "
...
class MalformedXML(KnownResponseError):
    message = (
        "This happens when the user sends malformed xml (xml that "
...
class MissingRequestBodyError(KnownResponseError):
    message = (
        "This happens when the user sends an empty xml document "
...
class NoSuchVersion(KnownResponseError):
    message = (
        "Indicates that the version ID specified in the "
...
class UnresolvableGrantByEmailAddress(KnownResponseError):
    message = (
        "The email address you provided does not match any account "
        "on record."
    )
