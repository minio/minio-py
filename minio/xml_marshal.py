...
def xml_marshal_bucket_constraint(region):
    ...
    """
    root = s3_xml.Element(
        "CreateBucketConfiguration", {"xmlns": _S3_NAMESPACE}
    )
...
def xml_marshal_select(opts):
    ...
    if opts.in_ser.csv_input is not None:
        ...
        allow_quoted_record_delimiter.text = (
...
def xml_marshal_bucket_notifications(
    ...
    """
    root = s3_xml.Element(
        "NotificationConfiguration", {"xmlns": _S3_NAMESPACE}
    )
...
def _add_notification_config_to_xml(node, element_name, configs):
    ...
    for config in configs:
        ...
        arn_node = s3_xml.SubElement(
            config_node, NOTIFICATIONS_ARN_FIELDNAME_MAP[element_name]
        )
        ...
        filter_rules = (
            config.get("Filter", {}).get("Key", {}).get("FilterRules", [])
        )
        if filter_rules:
