...
from .models import (AbortMultipartUploadResponse, Checksum,
                     ...
                     PutObjectFanOutResponse, ReplicationConfig, Retention,
                     SelectObjectContentRequest, SelectObjectResponse,
                     SSEConfig, StatObjectResponse, Tagging, Tags,
                     UploadPartCopyResponse, UploadPartResponse,
                     VersioningConfig, parse_list_objects)
from .signer import presign_v4, sign_v4_s3
from .sse import Sse, SseCustomerKey, check_sse, check_ssec
...
class Minio:
    ...
    def select_object_content(
        ...
        check_bucket_name(bucket_name, s3_check=self._base_url.is_aws_host)
        check_object_name(object_name)
        if not isinstance(request, SelectObjectContentRequest):
            raise ValueError("request must be SelectObjectContentRequest type")
    ...
    def set_bucket_cors(
        ...
        """
        check_bucket_name(bucket_name, s3_check=self._base_url.is_aws_host)
        if not isinstance(config, CORSConfig):
            raise ValueError("config must be CORSConfig type")
        body = marshal(config)
        headers = HTTPHeaderDict(
            {"Content-MD5": base64_string(MD5.hash(body))},
        )
    ...
    def prompt_object(
        ...
        Returns:
            HTTPResponse:
                An :class:`minio.compat.HTTPResponse` object.
