...
class MakeBuckets(TestCase):
    ...
    def test_make_buckets_all_success(self, mock_connection):
        ...
        client = Minio('localhost:9000')
        results = client.make_buckets(bucket_names)
    ...
    @mock.patch('urllib3.PoolManager')
    def test_make_buckets_with_failure_fail_on_error_true(self, mock_connection):
        error_xml = generate_error('BucketAlreadyExists', 'The requested bucket name is not available',
        ...
        self.assertRaises(S3Error, client.make_buckets, ['bucket1', 'bucket2', 'bucket3'])

    @mock.patch('urllib3.PoolManager')
    def test_make_buckets_with_failure_fail_on_error_false(self, mock_connection):
        error_xml = generate_error('BucketAlreadyExists', 'The requested bucket name is not available',
                                   'request_id', 'host_id', 'resource', 'bucket2', 'object')
        mock_server = MockConnection()
        mock_connection.return_value = mock_server
        ...
        client = Minio('localhost:9000')
        results = client.make_buckets(bucket_names, fail_on_error=False)
    ...
    @mock.patch('urllib3.PoolManager')
    def test_make_buckets_with_location_and_object_lock(self, mock_connection):
        mock_server = MockConnection()
        mock_connection.return_value = mock_server
        ...
        client = Minio('localhost:9000')
        results = client.make_buckets(bucket_names, location='us-west-1', object_lock=True)
